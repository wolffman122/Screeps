...
export class LinkHarvesterLifetimeProcess extends LifetimeProcess {
  ...
  run() {
    ...
    if (this.kernel.data.roomData[source.room.name].sourceLinkMaps[source.id]) {
      ...
      if (creep.name === 'em-E56S43-26263720')
        console.log(this.name, 1, creep.carry, creep.store.getUsedCapacity(), creep.carryCapacity, creep.store.getUsedCapacity() == creep.carryCapacity, creep.store.getUsedCapacity() === creep.carryCapacity);
