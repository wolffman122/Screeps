...
var getMapPath = function(start_room: string, end_room: string, opts: TravelOptions) {
    _.defaults(opts,{
    ...
    let ret = Game.map.findRoute(start_room,end_room, {
        routeCallback: (roomName) => {
            if (opts.avoidRooms.length > 0) {
                ...
                if (opts.avoidRooms.indexOf(roomName) > -1) {
            ...
            if (opts.preferRooms.length > 0) {
                if (opts.preferRooms.indexOf(roomName) > -1) {
